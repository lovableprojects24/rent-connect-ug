...
import { payments, formatUGX } from '@/data/mock-data';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-3">
        <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="flex items-center gap-3 bg-card border border-border rounded-xl p-4 hover:shadow-md transition-shadow text-left">
        ...
        <motion.button initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }} className="flex items-center gap-3 bg-card border border-border rounded-xl p-4 hover:shadow-md transition-shadow text-left">
            ...
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">{payment.propertyName} · {payment.unitName}</p>
                </div>
              </div>
              <StatusBadge status={payment.status} />
            </div>
            <div className="flex items-center justify-between mt-3 text-sm">
              <span className="text-muted-foreground">{payment.date} · {payment.method}</span>
          ...
          <tbody>
            {payments.map((payment) => (
              <tr key={payment.id} className="border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
                <td className="px-4 py-3 text-sm font-medium">{payment.tenantName}</td>
                <td className="px-4 py-3 text-sm text-muted-foreground">{payment.propertyName} · {payment.unitName}</td>
                <td className="px-4 py-3 text-sm font-semibold">{formatUGX(payment.amount)}</td>
                <td className="px-4 py-3 text-sm">
                  <span className="flex items-center gap-1.5">{methodIcons[payment.method]} {payment.method}</span>
                </td>
