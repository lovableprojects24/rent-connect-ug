...
import {
  ...
  Home, Wrench, Bell, ChevronRight, BarChart3, UserPlus,
} from 'lucide-react';
...
export default function LandlordAdminDashboardPage() {
    ...
    <div className="space-y-6">
      ...
      <motion.div {...anim(0)} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">Here's an overview of your properties and operations.</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <motion.div {...anim(1)}>
          <StatCard title="My Properties" value={computed.totalProperties} icon={Building2} trend={{ value: `${computed.totalUnits} units`, isPositive: true }} variant="primary" />
        </motion.div>
        <motion.div {...anim(2)}>
        ...
        <motion.div {...anim(4)}>
          <StatCard title="Maintenance" value={computed.openMaintenance} icon={Wrench} trend={{ value: 'open tickets', isPositive: false }} variant="warning" />
        </motion.div>
      </div>
            ...
            <CardContent className="space-y-3">
              {computed.propertyStats.length === 0 ? (
                ...
                computed.propertyStats.map((prop) => (
                  <Link key={prop.id} to={`/properties/${prop.id}`} className="flex items-center gap-3 p-3 rounded-xl border border-border bg-muted/30 hover:bg-muted/60 transition-colors">
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex flex-wrap gap-2">
              <Button variant="outline" size="sm" asChild>
                <Link to="/properties" className="gap-1.5"><Building2 className="w-4 h-4" /> My Properties</Link>
              </Button>
              <Button variant="outline" size="sm" asChild>
                <Link to="/tenants" className="gap-1.5"><Users className="w-4 h-4" /> Tenants</Link>
              </Button>
              <Button variant="outline" size="sm" asChild>
                <Link to="/staff" className="gap-1.5"><UserPlus className="w-4 h-4" /> My Staff</Link>
              </Button>
              <Button variant="outline" size="sm" asChild>
